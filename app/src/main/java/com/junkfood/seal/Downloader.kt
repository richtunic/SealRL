...
object Downloader {
    ...
    fun getInfoAndDownload(
        ...
        currentJob =
            applicationScope.launch(Dispatchers.IO) {
                val historyRecord = DatabaseUtil.isUrlAlreadyDownloaded(url)
                if (historyRecord != null) {
                    ToastUtil.makeToastSuspend(context.getString(R.string.status_completed))
                    val text = context.getString(R.string.download_finish_notification)
                    val notificationId = url.hashCode()
                    FileUtil.createIntentForOpeningFile(historyRecord.videoPath).run {
                        NotificationUtil.finishNotification(
                            notificationId,
