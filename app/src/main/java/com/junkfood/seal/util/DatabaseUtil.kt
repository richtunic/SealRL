...
object DatabaseUtil {
    ...
    suspend fun getInfoByUrl(url: String): DownloadedVideoInfo? =
        dao.getInfoByUrl(url).firstOrNull()

    suspend fun isUrlAlreadyDownloaded(url: String): DownloadedVideoInfo? {
        val history = getDownloadHistory()
        return isUrlAlreadyDownloaded(url, history)
    }
    ...
    fun isInstagramItemDownloaded(itemId: String, history: List<DownloadedVideoInfo>): Boolean {
